...
int main() {
	...
	for (int i{}; i < T; ++i) {
		...
		Node* curMax;
		Node* curMin;
		if (tree.find(x)->leftChild == NULL) {
			curMax = tree.find(x);
		}
		else {
			curMax = tree.max_findNode(tree.find(x)->leftChild);
		}

		if (tree.find(x)->rightChild == NULL) {
			curMin = tree.find(x);
		}
		else {
			curMin = tree.min_findNode(tree.find(x)->rightChild);
		}
